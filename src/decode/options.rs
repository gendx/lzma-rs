...
pub enum UnpackedSize {
    /// Assume that the 8 bytes used to specify the unpacked size are present in
    /// the header. If the bytes are `0xFFFF_FFFF_FFFF_FFFF`, assume that
    /// there is an end-of-payload marker in the file.
