...
#[inline(always)]
fn get_u64_le(buf: &[u8; 8]) -> u64 {
    (buf[0] as u64) | ((buf[1] as u64) << 8) | ((buf[2] as u64) << 16) |
        ((buf[3] as u64) << 24) | ((buf[4] as u64) << 32) | ((buf[5] as u64) << 40) |
        ((buf[6] as u64) << 48) | ((buf[7] as u64) << 56)
}

pub fn read_u64_le<R: io::Read>(stream: &mut R) -> io::Result<u64> {
    let mut buf = [0; 8];
    stream.read_exact(&mut buf)?;
    Ok(get_u64_le(&buf))
}
...
fn set_u64_le(buf: &mut [u8; 8], val: u64) {
    buf[0] = val as u8;
    buf[1] = (val >> 8) as u8;
    buf[2] = (val >> 16) as u8;
    buf[3] = (val >> 24) as u8;
...
pub fn write_u64_le<W: io::Write>(stream: &mut W, val: u64) -> io::Result<()> {
    let mut buf = [0; 8];
    set_u64_le(&mut buf, val);
    stream.write_all(&buf)
}
